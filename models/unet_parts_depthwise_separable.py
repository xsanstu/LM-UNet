...
class DoubleShuffledConvDS(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=None, kernels_per_layer=1, norm_layer=nn.BatchNorm2d,
                 activation=nn.ReLU, pw_groups=1):
        super().__init__()
        if not mid_channels:
...
class DownShuffledDS(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernels_per_layer=1, norm_layer=nn.BatchNorm2d, activation=nn.ReLU,
                 pw_groups=1):
        super().__init__()
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleShuffledConvDS(in_channels, out_channels, kernels_per_layer=kernels_per_layer, norm_layer=norm_layer,
                                 activation=activation, pw_groups=pw_groups),
        )
...
class UpShuffledDS(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True, ddf=False, kernels_per_layer=1, partial=False,
                 norm_layer=nn.BatchNorm2d, activation=nn.ReLU, pw_groups=1):
        super().__init__()
        ...
        if bilinear:
            ...
            self.conv = DoubleShuffledConvDS(in_channels, out_channels, kernels_per_layer=kernels_per_layer,
                                             norm_layer=norm_layer, activation=activation, pw_groups=pw_groups)
