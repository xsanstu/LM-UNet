...
class VSSMEncoder(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, depths=[2, 2, 9, 2],
        ...
        for i_layer in range(self.num_layers):
            layer = VSSLayer(
                dim=dims[i_layer],          # [96, 192, 384, 768]
                depth=depths[i_layer],      # [2, 2, 9, 2]
...
class UNetResDecoder2(nn.Module):
    def __init__(
            self,
            num_classes: int,
            deep_supervision: bool = False,
        ...
        encoder_output_channels = features_per_stage        # [64 128 256 512]
        self.deep_supervision = deep_supervision
        self.num_classes = num_classes
        n_stages_encoder = len(encoder_output_channels)
        ...
        concat_back_dim = []
        for s in range(1, n_stages_encoder):
            input_features_below = encoder_output_channels[-s]          # [512 256 128 64]
            input_features_skip = encoder_output_channels[-(s + 1)]     # [256 128 64]
            expand_layers.append(PatchExpand(
                input_resolution=None,
                dim=input_features_below,
                dim_scale=2,
        ...
        # expand_layers.append(PatchExpand(
        #     input_resolution=None,
        #     dim=encoder_output_channels[0],     # 64
        #     dim_scale=2,
        #     norm_layer=nn.LayerNorm,
        # ))
        # # expand_layers.append(FinalPatchExpand_X4(
        ...
        self.seg_layers = nn.Conv2d(input_features_skip, num_classes, 1, 1, 0, bias=True)
    ...
    #     lres_input = skips[-1]
    #     for s in range(len(self.stages)):
    #         x = self.expand_layers[s](lres_input)   # 宽度和高度变为原来的2倍，通道数减半，（上采样的感觉）线性变换为啥不是卷积
    #         if s < (len(self.stages) - 1):
    #             x = torch.cat((x, skips[-(s+2)].permute(0, 2, 3, 1)), -1)
    #             x = self.concat_back_dim[s](x)
    #         lres_input = self.stages[s](x).permute(0, 3, 1, 2)
    #     # x = self.expand_layers[-1](lres_input)
    #     # lres_input = self.stages[-1](x).permute(0, 3, 1, 2)
    #
    #     return lres_input

    def forward(self, skips):
        sk1, sk2, sk3, sk4, bt = skips[0], skips[1], skips[2], skips[3], skips[4]
        expand1 = self.expand_layers[0](bt)
        cat1 = torch.cat((expand1, sk4.permute(0, 2, 3, 1)), -1)
        cat_back1 = self.concat_back_dim[0](cat1)
        stage1 = self.stages[0](cat_back1).permute(0, 3, 1, 2)

        expand2 = self.expand_layers[1](stage1)
        cat2 = torch.cat((expand2, sk3.permute(0, 2, 3, 1)), -1)
        cat_back2 = self.concat_back_dim[1](cat2)
        stage2 = self.stages[1](cat_back2).permute(0, 3, 1, 2)

        expand3 = self.expand_layers[2](stage2)
        cat3 = torch.cat((expand3, sk2.permute(0, 2, 3, 1)), -1)
        cat_back3 = self.concat_back_dim[2](cat3)
        stage3 = self.stages[2](cat_back3).permute(0, 3, 1, 2)

        expand4 = self.expand_layers[3](stage3)
        cat4 = torch.cat((expand4, sk1.permute(0, 2, 3, 1)), -1)
        cat_back4 = self.concat_back_dim[3](cat4)
        stage4 = self.stages[3](cat_back4).permute(0, 3, 1, 2)
