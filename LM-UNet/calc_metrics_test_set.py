...
def get_metrics_from_model(model, test_dl, threshold=0.5, device: str = "cpu"):
    ...
    with torch.no_grad():
        ...
        for x, y_true in tqdm(test_dl, leave=True):
            # Move data to device
            x = x.to(device)
            y_true = y_true.to(device)      # (1, 12, 288, 288)
            y_pred = model(x)               # (1, 12, 288, 288)
            ssim_score += Ps(y_true.unsqueeze(0), y_pred)
            # hu添加  对于多输出通道选择其中的一个通道进行分析  只是单通道输出的不需要这一部分
            y_true = y_true.squeeze()       # (12, 288, 288)
            y_pred = y_pred.squeeze()       # (12, 288, 288)
            # y_true = y_true[-19,:,:]         # (288,288)
            # y_pred = y_pred[-19,:,:]         # (288,288)
            ...
            total_tn += tn
            total_fn += fn
            # ssim_score += calculate_ssim(y_pred_adj, y_true_adj)
        ...
        mse_ssim_score = ssim_score / len(test_dl)
...
def calculate_metrics_for_models(model_folder, threshold: float = 0.5):
    dataset = dataset_precip.precipitation_maps_oversampled_h5(
        in_file="/media/astu/新加卷/Nowcasting/KNMI/train_test_2016-2019_input-length_24_img-ahead_6_rain-threshold_20.h5",
        num_input_images=12,
        num_output_images=1,
        train=False,
    )
    ...
    for model_file in tqdm(models, desc="Models", leave=True):
        ...
        mse_image, mse_denormalized_image, mse_pixel, precision, recall, accuracy, f1, csi, far, hss,ssim_score = (
...
if __name__ == "__main__":
    ...
    # model_folder = ROOT_DIR / "checkpoints" / "NL50ckpt"
    # model_folder = ROOT_DIR / "checkpoints" / "NL20ckpt"
    model_folder = ROOT_DIR / "checkpoints" / "comparison"
    threshold = 0.5

    test_metrics_file = model_folder / f"model_metrics_{threshold}mmh_th20.txt"
    # test_metrics_file = model_folder / f"model_metrics_mmh.txt"
    if load_metrics:
